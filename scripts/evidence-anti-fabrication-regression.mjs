  ...
  const checks = [
    ...
    test('no unsupported reputation-count shorthand in public source', () => {
      const forbiddenPatterns = [
        ...
        /handles\s+(?:the\s+)?(?:entire|full|all)\s+permit(?:ting)?\s+process/i,
        /we\s+handle\s+(?:the\s+)?(?:entire|full|all)\s+permit(?:ting)?\s+process/i,
        /at\s+no\s+additional\s+charge/i,
        /highest\s+level\s+of\s+craftsmanship/i,
        /full\s+protection\s+of\s+Trex/i,
        /handles\s+design,\s+permits/i,
        /handles\s+all\s+permitting\s+in-house/i,
        /Trex\s+Platinum/i,
        /TrexPro\s+Platinum/i,
        /TimberTech\s+Certified/i,
        /Certified\s+Installer/i,
        /certified\s+Trex/i,
        /permit\s+handling/i,
        /full\s+permit/i,
        /entire\s+permit/i,
        ...
        /handled\s+in-house/i,
        /at\s+no\s+extra\s+cost/i,
        /Free\s+24h/i,
        /Free\s+estimate\s+in\s+24h/i,
        /25-50\s+year\s+warranties/i,
        /best\s+warranties/i,
      ];
      const offenders = [];
      for (const file of sourceFiles()) {
