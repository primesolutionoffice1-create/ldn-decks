# Proof System Staging Manifest — 2026-06-19
...
- `npm run seo:evidence-sprint:validate` must pass before owner evidence sprint blockers are staged.
- `npm run seo:evidence-action-packet` must regenerate owner action packets before staging.
- `npm run seo:evidence-action-packet:validate` must pass before owner action packets are staged.
- `npm run seo:proof-source-checklist:validate` must pass before proof-source checklist wording is staged.
- `npm run seo:proof-ops-board:validate` must pass before GBP, Maps, citation, or proof-ops tasks are staged.
...
- `npm run seo:commercial-intake:validate` must pass before warranty or repair-cost intake rows are staged.
- `npm run dataforseo:n8n-mcp:all` must pass before DataForSEO operator handoff files are staged.
- `npm run scaling:evidence-request:validate` must pass before scaling evidence request packets are staged.
- `npm run scaling:evidence-bundle:validate` must pass before scaling evidence bundles are staged.
- `npm run scaling:blocker-exit:validate` must pass before scaling blocker exit checklists are staged.
...
- Missing asset requirements: `12/12`
- Warranty terms: `0`
...
## Verification Commands To Re-run Before Staging

Use this checklist on 2026-06-19 before staging or handing off proof work. The manifest records required commands; command output remains the source of truth for pass/fail status.
...
- `npm run seo:proof-source-checklist:validate` — must pass before proof-source checklist wording is staged.
- `npm run seo:proof-ops-board:validate` — must pass before GBP, Maps, citation, or proof-ops tasks are staged.
- `npm run seo:directory-citations:validate` — must pass before citation URL or `sameAs` changes are staged.
- `npm run seo:validate-ai-discovery` — must pass before AI discovery files or routes are staged.
- `npm run seo:proof-commands:validate` — must pass before proof docs are handed to the owner.
- `npm run seo:commercial-intake:validate` — must pass before warranty or repair-cost intake rows are staged.
- `npm run dataforseo:n8n-mcp:all` — must pass before DataForSEO operator handoff files are staged.
...
- `npm run scaling:evidence-bundle:validate` — must pass before scaling evidence bundles are staged.
- `npm run scaling:blocker-exit:validate` — must pass before scaling blocker exit checklists are staged.
- `npm run seo:evidence-regression` — must pass before proof gate core is staged.
- `npm run seo:validate-owner-intake` — must pass before owner intake artifacts are staged.
- `npm run seo:validate-evidence` — must pass before ledger-related proof files are staged.
- `npm run seo:weekly` — must pass before final handoff.
- `npm run build` — must pass after the final owner evidence import and before deploy.
- `npm run seo:prepublish-evidence` — is expected to fail until owner evidence lands; it must pass before merge/deploy.
