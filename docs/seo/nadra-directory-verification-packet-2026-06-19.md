...
| Official NADRA logo asset | Evidence needed: original or NADRA-approved logo file is not yet archived in `public/badges/`. |
...
1. Open the supplied NADRA profile URL: `https://www.nadra.org/membership/directory#!biz/id/69f274b54078d1282501ee3b`.
...
6. Upload the original or NADRA-approved Proud Member logo asset to `public/badges/` before replacing the current text badge with an image logo.
7. Archive the screenshot path in the owner proof ledger before treating the citation as fully resolved.
...
| Official NADRA logo file | Yes, before visual-logo rollout | Must be an original/NADRA-approved asset, not recreated or manually designed. |
| Profile details/contact fields | Optional | Useful if NADRA exposes NAP, phone, email, or website. |

## Proof Ops Acceptance Criteria

The NADRA task can stay `owner-verify-live` when the supplied profile URL is linked from the NADRA badge, the profile URL is recorded in `src/lib/business.js`, and the remaining work is screenshot archival.

The task can move to `resolved` only when a public NADRA profile or member-dashboard screenshot is archived, the screenshot shows the Loudoun Decks membership context, and the website badge link is confirmed against the supplied profile URL.
