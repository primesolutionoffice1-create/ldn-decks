...
| Core cities | Ashburn, Leesburg, Sterling, Brambleton, South Riding, Reston, Herndon, Fairfax, Vienna, McLean, Manassas, Gainesville, Haymarket, Centreville |

## Suggested Public Description

Use only if Nextdoor allows a business description field:

```text
Loudoun Decks is a Northern Virginia deck and outdoor living contractor based in Centreville, VA. We help homeowners plan and build custom composite decks, wood decks, deck resurfacing projects, screened porches, patios, pergolas, railings, and related outdoor living upgrades across Loudoun County, Fairfax County, Prince William County, Arlington, and Stafford.
```

Do not add review counts, “#1” claims, fake project counts, unverifiable warranty terms, or manufacturer credential claims unless the profile has a separate verified proof source.
...
The Nextdoor cleanup task can move from `owner-needed` to `owner-verify-live` only when:

- The public profile or admin screenshot shows `Loudoun Decks` as the primary business name.
- The profile no longer uses `loudoun decks buildin` as the primary name.
- The email is `office@ldndecks.com`, or the old email is hidden from public view.
- The website is `https://ldndecks.com/` without ad-tracking parameters.
- The category mix is relevant to decks/outdoor living.
- Screenshots are stored in the proof archive or attached to the owner evidence packet.
