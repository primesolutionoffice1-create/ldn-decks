...
The Apple Business Connect task can move from `owner-needed` to `owner-verify-live` when the owner has dashboard access or a support/verification request is submitted, a screenshot shows the Apple place/profile record, and the screenshot shows canonical NAP or clearly shows the fields are pending correction.

The task can move to `resolved` only when Apple Business Connect shows the profile as verified or active, canonical NAP is visible, a public Apple Maps place card is available or the dashboard confirms the live Apple Maps listing, and proof screenshots are archived.

## SameAs Policy

Add an Apple Maps URL to `src/lib/business.js` `sameAs` only after the public place card URL is known, public or dashboard evidence shows canonical `Loudoun Decks` identity, and no old Prime Solutions/Ashburn/LDN variant is exposed as the primary public entity.
