...
| Loudoun Chamber | https://business.loudounchamber.org/list/member/loudoun-decks-30047 | 200 | Yes | Public page is corrected to `Loudoun Decks`, `13704 Winding Oak Cir`, `Centreville, VA 20121`, phone `(571) 655-7207`, and has a direct `Visit Website` link to `ldndecks.com`. | High-value backlink; keep member profile active and consider event/sponsorship links. |
| BuildZoom | https://www.buildzoom.com/contractor/loudoun-decks | 200 | No public crawlable link found yet | Page exists with correct Centreville NAP, description, and photos. Admin now has `https://ldndecks.com/`, Facebook, and Yelp set, and project preferences were narrowed to deck/outdoor-living categories. Public HTML still did not expose `ldndecks.com` during the last fetch. Public profile still shows license `2705191673`, status `Unable To Verify`, and `No active license on file`; license is pending BuildZoom review since 2025-11-15. | Contact BuildZoom Contractor Relations / finish license-board verification; re-fetch public page after BuildZoom cache updates. |
| Bing Places | https://www.bing.com/maps?ss=ypid.YND1AFC4105F09B172&mkt=en-US | Live in dashboard | Yes | Bing Places listing is published and synced with Google. NAP and website are correct; `office@ldndecks.com` was added on 2026-05-16. Dashboard showed 87 views for Apr 13-May 12, 2026. Yext scan incorrectly reported Bing missing. | Keep synced with Google; category cleanup must happen in Google first because Bing locks synced fields. |
...
- Verified Loudoun Chamber as a real crawlable backlink:
  `https://business.loudounchamber.org/list/member/loudoun-decks-30047` links directly to `https://ldndecks.com`.
  ChamberMaster admin was updated on 2026-05-16 from singular `Loudoun Deck` and Ashburn NAP to `Loudoun Decks`, `13704 Winding Oak Cir`, `Centreville, VA 20121`; public page now reflects the corrected profile.
- Added verified authority profiles to organization `sameAs`: BBB Centreville, Trustpilot, and Loudoun Chamber.
...
- Verified Bing Places on 2026-05-16. The listing is already published and synced with Google, so it is not actually missing despite the Yext scan. Added `office@ldndecks.com` to the Bing listing.
- Updated BuildZoom admin on 2026-05-16: website `https://ldndecks.com/` was present, Facebook was present, Yelp was added as `https://www.yelp.com/biz/loudoun-decks-centreville`, and project/service preferences were narrowed to framing, decks and porches, patios, and pergolas/arbors. Removed broad categories like general remodel, commercial renovation, roofing, flooring, masonry, stone/tile, pool/spa, site work, and other generic services. License `2705191673` remains pending BuildZoom review and the public page still needs BuildZoom to remove the `Unable To Verify` / `No active license on file` warning.
...
- BuildZoom: admin profile is now updated with website, Facebook, Yelp, description, photos, and narrower service categories. Remaining blocker is BuildZoom/license verification: public profile still shows `Unable To Verify` / `No active license on file`, which can reduce conversion confidence. Call Contractor Relations at `(844) 329-5737` or use BuildZoom support to push license-board verification.
- Bing Places: listing is already published and synced with Google; NAP/website are correct and email has been added. If categories need improvement, update Google Business Profile first, then resync Bing.
- MapQuest/Yext: scan submitted on 2026-05-16. The `Fix all errors` / `Fix my listings now` path returned to a Yext scan/contact flow rather than a direct free correction screen. Do not purchase paid Yext without approval. Use the scan output as a citation cleanup map: MapQuest, Citysearch, Foursquare, Insider Pages, eLocal, Tupalo, Navmii, Opendi, iGlobal/iBegin are missing; Facebook has a phone/name variant; MerchantCircle, EZlocal, CitySquares, USCity.net, GoLocal247, and AroundMe show old `Prime Solutions` / Ashburn-style contamination; ShowMeLocal, Where To?, and Cylex use name variants. Treat Bing as verified despite Yext's scan result.
- Growcycle: public result shows `ldndecks.com/` but also old `42785 Generation Dr, Ashburn, VA 20147`; claim/update or suppress because it conflicts with the canonical Centreville NAP.
- Nextdoor: public result shows a name variant (`loudoun decks buildin`) and older Gmail-style contact; claim/update if account access is available.
