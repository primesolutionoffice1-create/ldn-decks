...
| Action | Platform | Source | Owner | Confidence | Impact | Risk | Approval | Rollback |
|---|---|---|---|---|---|---|---|---|
| Monitor post-cleanup conversion goals through 5-10 real production leads. | google | ../../../../docs/ads-tracking/GOOGLE-ADS-CONVERSION-CLEANUP-LOG.md; ../../../../docs/tracking-audit/FINAL-ATTRIBUTION-SIGNOFF.md | LDN Decks | high | critical | `Submit lead form` and `Phone call lead` are now the account-default optimization path, but diagnostics still need real-lead evidence. | needs_review | Re-enable prior goal settings only if form/call tracking is disproven; do not change budgets or bid strategies. |
