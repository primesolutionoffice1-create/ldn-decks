...
- One site only: `https://ldndecks.com`.
- Any other sites Ldndecks owns are explicitly out of scope for this sprint, regardless of niche or condition.
...
- Over-promising specific numbers erodes trust between Ldndecks and Daniel Agrici and pushes the work toward cosmetic refreshes that look productive but do not address the underlying trust drag.
- Honest scope-setting upfront also gives Ldndecks permission to prune aggressively when the audit calls for it, instead of hoarding off-niche pages out of fear of losing residual traffic.

## Acceptance

Ldndecks acknowledges this scope before the sprint starts. Acknowledgement recorded in `[[Log]]` with date.

## Related

- Operational decision tree: [[Content Pruning Decision Framework]]
- Hypothesis driving the diagnostic: [[Pre-Audit Hypothesis]]
- Onboarding sequence Ldndecks follows: [[Start Here]]
