...
Hidden monetization is one of the clearest signals quality raters and the HCU classifier are tuned to detect. This decision sets the disclosure bar for every page on `https://ldndecks.com` that contains an affiliate link.
...
- Visible disclosure signals editorial honesty and helps establish the trust signal Ldndecks is trying to build / recover.
...
- Owner: Ldndecks (executes on every affiliate page).
- Verifier: Daniel Agrici (spot-checks during refresh phase).
