...
- **Pathway D — Outbound click to third-party booker / scheduler / partner.** GA4 event `external_referral` with parameters `target_platform` and `source_url`. Whether this pathway is live depends on Ldndecks's actual booking infrastructure.
...
- **Owner:** Ldndecks — configures GA4, the form, the click tracking, and the consent layer.
- **Verifier:** Daniel Agrici — confirms each event fires correctly in GA4 DebugView before any consolidation ships, and confirms the cluster pivot exploration is saved.
...
The following items in [[Open Questions for Ldndecks]] block full execution of this plan and must be resolved before Acceptance can pass:
...
These are flagged as blocking. Do not assume defaults; confirm with Ldndecks before configuring.
...
- [[Open Questions for Ldndecks]] — where the blocking dependencies live.
