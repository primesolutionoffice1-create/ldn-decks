...
This roadmap assumes **Day 0 starts the day Ldndecks completes [[Day 0 Measurement Access Gate]]**. All downstream dates shift if Day 0 slips. Days are working-days indicative, not strict calendar days — if Ldndecks is part-time, the calendar window stretches accordingly.

Owner per slice is named below. Verifier is Daniel Agrici for every slice unless otherwise specified. Acceptance criteria carry from each linked flow.
...
Owner: Ldndecks. Until this gate closes, every recommendation in this brain is advisory.
...
- Install `claude-seo`.
- Run `/seo-audit` against `https://ldndecks.com`.
...
Owner: Ldndecks (with Daniel Agrici scaffolding). Parallel to GSC export — does not block the loss-map work.
...
Owner: Daniel Agrici (analysis), Ldndecks (CMS fixes).
...
Owner: Ldndecks (CMS execution), Daniel Agrici (verification).
...
Owner: Ldndecks (with named expert input on first-person passages and proof artifacts).
...
- HCU recovery typically realizes at the next Google core update window. Per [[Recovery Scope and Expectations]], do not promise a date — set Ldndecks's expectations correctly.
